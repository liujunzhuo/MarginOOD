...
def plot_distribution(args, id_scores, ood_scores, out_dataset):
    ...
    plot = sns.displot({"ID": -1 * id_scores, "OOD": -1 * ood_scores}, label="id", kind="kde", palette=palette, fill=True,
                alpha=0.8).set(xlim=(-1.5, 0), ylim=(0, 3))
    plt.savefig(os.path.join(args.log_directory, f"KNN_{out_dataset}.svg"), bbox_inches='tight', format='svg')
...
def save_as_dataframe(args, out_datasets, fpr_list, auroc_list, aupr_list):
    ...
    data['AVG(5)'] = [np.mean(fpr_list[:-1]), np.mean(auroc_list[:-1]), np.mean(aupr_list[:-1])]
    data['AVG(5)'] = [float('{:.2f}'.format(metric)) for metric in data['AVG(5)']]
